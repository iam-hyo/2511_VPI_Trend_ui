...
function TrendVideoRow({ video, rank }) {
  ...
  const handleGenerateScript = async () => {
    ...
    // [수정] 선택(Set) -> 관련 비디오 4개의 ID 배열
    const videoIds = relatedVids.map(v => v.id.videoId || v.id);

    try {
      const data = await generateScript(videoIds, video.keyword);
